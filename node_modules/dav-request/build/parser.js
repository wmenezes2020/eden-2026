...
let debug = (0, _debug.default)('dav:parser');
let DOMParser;
...
let traverse = {
  // { response: [x, y, z] }
  multistatus: node => complex(node, {
    response: true
  }),
  // { propstat: [x, y, z] }
  response: node => complex(node, {
    propstat: true,
    href: false
  }),
  // { prop: x }
  propstat: node => complex(node, {
    prop: false
  }),
  // {
  //   resourcetype: x
  ...
  // [x, y, z]
  supportedCalendarComponentSet: node => complex(node, {
    comp: true
  }, 'comp'),
  // [x, y, z]
  ...
  // x
  supportedReport: node => complex(node, {
    report: false
  }, 'report'),
  report: node => {
    return childNodes(node).map(childNode => childNode.localName);
...
function complex(node, childspec, collapse) {
  ...
  childNodes(node).forEach(childNode => traverseChild(node, childNode, childspec, result));
...
function traverseChild(node, childNode, childspec, result) {
  ...
  let localName = (0, _camelize.default)(childNode.localName, '-');
